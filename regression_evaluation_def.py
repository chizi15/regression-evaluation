...
def rmse(y_true, y_pred):
    return round(np.sqrt(((y_true - y_pred) ** 2).mean()), 4)
...
def emlae(y_true, y_pred):
    ...
    return round(np.exp(sum(np.log(abs(y_pred - y_true) + 1)) / n) - 1, 4)
...
def regression_evaluation(y_true, y_pred):
    ...
    for i in range(len(y_true_trun)):
        ...
        R2.append(round(metrics.r2_score(y_true=y_true_trun[i], y_pred=y_pred_trun[i]), 4))  # R2可描述线性或非线性二维数据的相关性。y_true, y_pred的series中，至少要有≥2个点，否则会返回nan；r2_score也为极大化目标函数，值域为(-∞, 1]，越趋近1越好；与其余的极小化目标函数相反，它们的因变量是越小越好。
